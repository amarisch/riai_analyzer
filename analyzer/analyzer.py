...
def analyze_fn(netname, specname, epsilon):
...
def verify_all(netdir, imgdir):
    netfiles = [f for f in listdir(netdir) if isfile(join(netdir, f))]
    imgfiles = [f for f in listdir(imgdir) if isfile(join(imgdir, f))]
    ep_list = [0.005, 0.01, 0.05, 0.07, 0.10]

    for network in netfiles:
        for image in imgfiles:
            for epsilon in ep_list:
                print("%s %s %.3f" % (network, image, epsilon))
...
if __name__ == '__main__':
    ...
    if len(argv) == 3:
        netdir = argv[1]
        imgdir = argv[2]
        verify_all(netdir, imgdir)
    else:
        netname = argv[1]
        specname = argv[2]
        epsilon = float(argv[3])
        #c_label = int(argv[4])
        analyze_fn(netname, specname, epsilon)
